...
.stButton > button {
  ...
  box-shadow: 0 1px 6px rgba(243,110,33,.3) !important;
  transition: all .15s !important;
}
.stButton > button:hover {
  background: #D45E18 !important;
  box-shadow: 0 3px 12px rgba(243,110,33,.4) !important;
...
h2 { font-size: 1.15rem !important; font-weight: 600 !important; color: var(--rubi) !important; }
h3 { font-size: 1rem !important; font-weight: 600 !important; color: var(--ink) !important; }
...
.login-title {
  font-family: 'Instrument Serif', serif;
  font-size: 1.7rem; color: var(--rubi);
  margin-bottom: .2rem;
}
.login-sub { font-size: 13px; color: var(--ink-m); margin-bottom: 2rem; }
...
.kb-card:hover {
  ...
  box-shadow: 0 4px 16px rgba(51,0,26,.1);
  transform: translateY(-1px);
}
.kb-card-title {
  font-weight: 600; font-size: 13px; color: var(--rubi); margin-bottom: 6px;
}
.kb-card-meta { font-size: 11.5px; color: var(--ink-m); line-height: 1.75; }
.kb-sla-ok  { color: #0B7A5F; font-size: 11px; font-weight: 600; margin-top: 5px; }
.kb-sla-med { color: #7A5010; font-size: 11px; font-weight: 600; margin-top: 5px; }
.kb-sla-bad { color: #C41230; font-size: 11px; font-weight: 600; margin-top: 5px; }
.kb-motivo  { font-size: 11px; color: #8B1010; margin-top: 4px; font-style: italic; }
.kb-obs-badge {
  display: inline-block; background: var(--gold-l);
  color: var(--gold); border-radius: 4px; padding: 1px 7px;
  font-size: 10.5px; font-weight: 600; margin-top: 5px;
}
.kb-wrap { min-height: 100px; }
...
.pt thead tr { border-bottom: 2px solid var(--rubi); }
.pt th {
  text-align: left; padding: .55rem .8rem;
  font-size: 11px; font-weight: 700;
  text-transform: uppercase; letter-spacing: .07em; color: var(--ink-m);
}
.pt td { padding: .55rem .8rem; border-bottom: 1px solid var(--border-s); color: var(--ink); }
.pt tr:last-child td { border-bottom: none; }
.pt tr:hover td { background: var(--cream); }
.pt .total-row td { background: #FFF3EC; font-weight: 700; color: var(--rubi); border-top: 2px solid var(--gold); }
...
.sec-label {
  ...
  margin-bottom: 8px; margin-top: 16px; padding-left: 4px;
}
</style>
"""
...
KB_CSS       = {"Em aberto":"kb-aberto","Em andamento":"kb-andamento",
                "Travado":"kb-travado","Concluido":"kb-concluido","Cancelado":"kb-cancelado"}
...
def style_critical(row):
    if row['Dias de Atraso']>90: return ['background:#ffcccc;color:#990000;font-weight:bold']*len(row)
...
def comp_sort_key(comp_str):
    """Convert MM/YYYY to sortable tuple (YYYY, MM)."""
    try:
        parts=str(comp_str).strip().split("/")
...
def dialog_criar_atividade(uc_usina="", ger_usina=""):
    ...
    with st.form("dlg_nova_ativ", clear_on_submit=True):
        c1,c2=st.columns(2)
        uv=c1.text_input("Usina",  value=uc_usina,  disabled=bool(uc_usina))
        gv=c2.text_input("Gerador",value=ger_usina, disabled=bool(ger_usina))
        titulo=st.text_input("Título *")
        d1,d2=st.columns(2)
        tipo =d1.selectbox("Tipo",TIPOS)
        agend=d2.text_input("Agendamento",placeholder="20/05/2026 09:00")
        desc =st.text_area("Descrição / Contexto inicial",height=90,
                           placeholder="Cole links HubSpot, tickets, notas de atendimento…")
        anex =st.file_uploader("Anexo opcional",type=["pdf","xlsx","xls"])
        ok=st.form_submit_button("Criar Atividade",use_container_width=True)
    if ok:
        if not titulo.strip(): st.warning("Título obrigatório."); return
        an,ab="",""
        if anex: an=anex.name; ab=base64.b64encode(anex.read()).decode()
        new_task(titulo.strip(),uv,gv,analista,tipo,agend,desc,an,ab)
        st.success("✅ Atividade criada. Acesse **Atividades**."); st.rerun()
...
def dialog_task_detail(tid):
    ...
    if not t: st.error("Não encontrado."); return
    ...
    if t.get('descricao') and t['descricao'].strip():
    ...
    st.caption("Registre atualizações, cole links HubSpot / tickets, notas de chamadas:")
    nova=st.text_area("Nova observação",height=110,
                      placeholder="Ex: Ligou para cliente 14/05 — aguardando doc. Ticket: https://…",
                      label_visibility="collapsed", key=f"obs_input_{tid}")
    if st.button("Registrar observação",key=f"sv_obs_{tid}"):
        if nova.strip():
            ts=datetime.now().strftime("%d/%m/%Y %H:%M")
            an=st.session_state["user"]["name"]
            novo_log=t.get("observacoes","")+f"\n[{ts} — {an}]\n{nova.strip()}"
            update_task(tid, observacoes=novo_log)
            st.success("Salvo."); st.rerun()
    if t.get("observacoes","").strip():
        with st.expander(f"Ver histórico de observações"):
            for bloco in t["observacoes"].strip().split("\n\n"):
                if bloco.strip(): st.text(bloco.strip()); st.markdown("---")
    ...
    if t.get("anexo_nome") and t.get("anexo_b64"):
        st.download_button(f"📎 {t['anexo_nome']}",
                           base64.b64decode(t["anexo_b64"]),t["anexo_nome"])

    st.markdown('<div class="sdiv"></div>',unsafe_allow_html=True)
    st.markdown("**Mover para:**")
    opcoes=[s for s in STATUS_LIST if s!=t["status"]]
    dest=st.selectbox("",opcoes,label_visibility="collapsed",key=f"dest_dlg_{tid}")
    mot=""
    if dest in MOTIVO_OBRIG:
        mot=st.text_area("Motivo obrigatório *",key=f"mot_dlg_{tid}")
    if st.button("Confirmar movimentação",use_container_width=True,key=f"mv_dlg_{tid}"):
        if dest in MOTIVO_OBRIG and not mot.strip():
            st.warning("O motivo é obrigatório."); return
        update_task(tid,status=dest,motivo_bloqueio=mot.strip()); st.rerun()
...
def render_sidebar():
    with st.sidebar:
        try: st.image("https://ops.sunne.com.br/static/media/logo-sunne.9e4fbe.png",width=110)
        except: st.markdown("**☀️ Sunne**")

        u=st.session_state["user"]
        st.markdown(f'<div class="sb-user"><div class="sb-user-name">{u["name"]}</div>'
                    f'<div class="sb-user-role">{u.get("role","user")}</div></div>',
...
def page_dashboard():
    ...
    tasks  =load_tasks(); geracao=load_geracao(); bo=load_backoffice()
    ...
    if res:
        with t2:
            for comp,items in res["missing"].items():
                with st.expander(f"⚠️ {comp} — {len(items)} faltantes"):
                    st.download_button(f"Baixar CSV ({comp})",
        ...
        with t3:
            for comp,rows in res["inad"].items():
                ...
                with st.expander(f"Clientes ({comp})"): st.table(pd.DataFrame(rows))
            st.markdown("---")
...
def page_geradores():
    ...
    with tc:
        ...
        c=[x for x in ["gerador","contato","concessionaria","usinas","porte","origem"] if x in df_s.columns]
        df_s=df_s[c]; df_s.columns=[x.capitalize() for x in c]
...
def page_usinas():
    ...
    if ca.button("Adicionar"): st.session_state["show_add_usi"]=not st.session_state.get("show_add_usi",False)
    if cb.button("Importar Planilha"): st.session_state["show_imp_usi"]=not st.session_state.get("show_imp_usi",False)
    ...
    if st.session_state.get("show_add_usi"):
        with st.form("fau",clear_on_submit=True):
            ...
            s,c=st.columns(2); sal=s.form_submit_button("Salvar"); can=c.form_submit_button("Cancelar")
        if sal:
            if not uc.strip(): st.warning("UC obrigatória.")
            else:
                us=load_usinas(); us.append({"uc":str(uc).strip(),"gerador":ger.strip(),
                    "ufv":ufv.strip(),"analista":an,"ativa":ativa,"geracao_estimada":gest,
                    "criado_em":datetime.now().strftime("%d/%m/%Y")})
                save_usinas(us); st.session_state["show_add_usi"]=False; st.rerun()
        if can: st.session_state["show_add_usi"]=False; st.rerun()
        ...
        if i1.button("Importar",key="btn_iusi") and f:
            try:
                ...
                for col in df.columns:
                    ...
                    if cl in ("geração","geracao","geração (kwh)","geracao (kwh)"): df.rename(columns={col:"geracao_estimada"},inplace=True)
                ...
                for _,row in df.iterrows():
                    ...
                    if uv and uv not in uc_ex:
                        us.append({"uc":uv,"gerador":str(row.get("gerador","")),
                            ...
                            "criado_em":datetime.now().strftime("%d/%m/%Y")})
                        uc_ex.add(uv); n+=1
                save_usinas(us); st.session_state["show_imp_usi"]=False; st.success(f"{n} usina(s)."); st.rerun()
            except Exception as e: st.error(str(e))
        if i2.button("Fechar",key="fusi"): st.session_state["show_imp_usi"]=False; st.rerun()
    ...
    st.markdown("<hr style='margin:4px 0 8px;border:none;border-top:1px solid var(--border)'>",unsafe_allow_html=True)
...
def page_atividades():
    ...
    hc1,hc2,_=st.columns([1.5,1.5,6])
    if hc1.button("Nova Atividade"):   dialog_criar_atividade()
    ...
    for col_ui,status in zip(cols,STATUS_LIST):
        ...
        with col_ui:
            st.markdown(
                f'<div class="kb-col-head {css}"><span>{status}</span>'
                f'<span style="background:rgba(0,0,0,.08);border-radius:12px;padding:1px 8px">{len(grupo)}</span></div>',
                unsafe_allow_html=True)
            ...
            for t in grupo:
                tid=t["id"]; dias=sla_days(t); sc=sla_cls(dias)
                ...
                if st.button("↗",key=f"mv_{tid}",help="Mover"):
                    if dest in MOTIVO_OBRIG:
                ...
                if st.session_state.get(f"pm_{tid}"):
                    ...
                    mot=st.text_area(f"Motivo *",key=f"mot_{tid}")
                    ...
                    if mc1.button("OK",key=f"conf_{tid}"):
                        ...
                        else: st.warning("Obrigatório.")
                    if mc2.button("✕",key=f"cno_{tid}"):
                        del st.session_state[f"pm_{tid}"]; st.rerun()

                st.markdown("<hr style='margin:4px 0;border:none;border-top:1px solid var(--border-s)'>",
                            unsafe_allow_html=True)
...
def page_geracao():
    ...
    with tm:
        # Campo de busca FORA do form para autocompletar em tempo real
        uc_inp=st.text_input("UC da Usina *",key="ger_uc_inp",placeholder="Digite a UC…")

        # Autocompletar nome em tempo real
        nome_auto=""
        gerador_auto=""
        if uc_inp.strip():
            usinas=load_usinas()
            match=next((u for u in usinas if normalize_uc(u["uc"])==normalize_uc(uc_inp)),None)
            if match:
                nome_auto=match.get("ufv","")
                gerador_auto=match.get("gerador","")
                st.success(f"✅ **{nome_auto}** · Gerador: **{gerador_auto}**")
            else:
                st.warning("UC não encontrada no cadastro. Prossiga com preenchimento manual.")

        with st.form("form_geracao",clear_on_submit=True):
            g1,g2=st.columns(2)
            nome_g=g1.text_input("Nome Usina",value=nome_auto,placeholder="Preenchido automaticamente")
            ...
            sald_g=g4.number_input("Saldo (kWh)",min_value=0.0,step=0.1)
            ok=st.form_submit_button("Salvar Geração",use_container_width=True)

        if ok:
            if not uc_inp.strip(): st.warning("UC obrigatória.")
            else:
                # Resolve nome automaticamente se ainda não preenchido
                usinas=load_usinas()
                m2=next((u for u in usinas if normalize_uc(u["uc"])==normalize_uc(uc_inp)),None)
                nome_final=m2.get("ufv","") if m2 else nome_g
                ger=load_geracao()
                ger.append({"uc":str(uc_inp).strip(),"nome_usina":nome_final,
                             "gerador":gerador_auto or (m2.get("gerador","") if m2 else ""),
                             "competencia":comp_g,"energia_injetada":inj_g,"saldo":sald_g,
                             "registrado_em":datetime.now().strftime("%d/%m/%Y %H:%M")})
                save_geracao(ger)
                st.success(f"Geração registrada — **{nome_final or uc_inp}**")
        ...
        if f and st.button("Importar",key="btn_iger"):
            try:
                ...
                for _,row in df.iterrows():
                    ...
                    ger.append({"uc":uv,"nome_usina":nm,"gerador":gd,
    ...
    # ── Registros — filtro por gerador e/ou usina, ordenado por competência
    st.markdown('<div class="sdiv"></div>',unsafe_allow_html=True)
    ...
    # Garantir colunas
    for col_n in ["uc","nome_usina","gerador","competencia","energia_injetada","saldo"]:
        if col_n not in df_g.columns: df_g[col_n]=""

    # Filtros
    fa,fb,_=st.columns([2,2,3])
    geradores_list=sorted({str(x) for x in df_g["gerador"].unique() if x})
    ger_filtro=fa.selectbox("Filtrar Gerador",["Todos"]+geradores_list,key="gf_ger")

    usinas_disponiveis=[]
    if ger_filtro!="Todos":
        usinas_disponiveis=sorted({str(r["nome_usina"]) for _,r in df_g[df_g["gerador"]==ger_filtro].iterrows() if r["nome_usina"]})
    usi_filtro=fb.selectbox("Filtrar Usina",["Todas"]+usinas_disponiveis,key="gf_usi",
                            disabled=(ger_filtro=="Todos"))

    df_f=df_g.copy()
    if ger_filtro!="Todos": df_f=df_f[df_f["gerador"]==ger_filtro]
    if usi_filtro!="Todas" and ger_filtro!="Todos": df_f=df_f[df_f["nome_usina"]==usi_filtro]

    # Ordenar por competência (MM/YYYY) mais recente primeiro
    df_f=df_f.copy()
...
def page_backoffice():
    ...
    nomes=[g["gerador"] for g in geradores]
    ...
    if f and st.button("Processar Extrato"):
        try:
            ...
            if not uc_c: st.error("Coluna UC não encontrada."); return
            bo=load_backoffice(); n_new=0; n_upd=0
            for _,row in df.iterrows():
                uv=str(row[uc_c]).strip()
                if not uv: continue
                rec={"uc":uv,"gerador":sel_ger,"titular":str(row[tt_c]) if tt_c else "—",
                ...
                if ix is not None: bo[ix]=rec; n_upd+=1
                else: bo.append(rec); n_new+=1
            save_backoffice(bo)
            st.success(f"✅ {n_new} novos · {n_upd} atualizados — Gerador: **{sel_ger}**")
        except Exception as e: st.error(str(e))
    ...
    if bo:
        ...
        gf=st.selectbox("Filtrar Gerador",["Todos"]+sorted({b.get("gerador","—") for b in bo}),key="bo_f")
    ...
    else: st.info("Nenhum dado ainda.")
...
def page_rateio():
    ...
    geradores=load_geradores(); nomes_ger=sorted({g["gerador"] for g in geradores})
    ...
    with ta:
        ...
        ug=get_usinas_do_gerador(sel_ger)
        if not ug:
            st.warning(f"Nenhuma usina vinculada a **{sel_ger}**. Verifique o campo Gerador nas Usinas."); st.stop()
        noms=[u.get("ufv") or u["uc"] for u in ug]
        sel_u=st.selectbox("Usina",noms,key="rb_usi")
        su=ug[noms.index(sel_u)]; uc_u=str(su["uc"])
        ...
        if not rg:
            st.warning(f"Geração de **{sel_u}** para {mes} não registrada. Registre em Geração primeiro."); st.stop()
        ...
        for i in range(int(ns)):
            c1,c2=st.columns(2)
            us=c1.text_input(f"UC #{i+1}",key=f"us_{i}"); ms=c2.text_input(f"Motivo #{i+1}",key=f"ms_{i}")
            if us: ucs_s.append({"uc":str(us).strip(),"motivo":ms})

        st.markdown("**Novas UCs:**")
        nn=st.number_input("Qtd",0,30,0,step=1,key="rb_nn")
        ucs_n=[]
        for i in range(int(nn)):
            c1,c2,c3=st.columns(3)
            un=c1.text_input(f"UC #{i+1}",key=f"un_{i}"); an2=c2.text_input(f"Apelido #{i+1}",key=f"an_{i}")
        ...
        if st.button("Calcular",key="btn_rb"):
            ...
            try:
                ...
                for _,rr in df_rat.iterrows():
                    un=rr["UC_NORM"]; ap=rr[ap_c] if ap_c else "—"; cn=rr[cn_c] if cn_c else "—"
                    er=df_ext[df_ext["UC_NORM"]==un]
                    if er.empty: ct=sa=0; tp="monofasico"
                    else:
                        er=er.iloc[0]; ct=clean_val(er[co_c]) if co_c else 0
                        sa=clean_val(er[sa_c]) if sa_c else 0; tp=str(er[ti_c]).lower() if ti_c else "monofasico"
                ...
                for nu in ucs_n:
                    ...
                    res.append({"UC_NORM":normalize_uc(nu["uc"]),"UC":nu["uc"],"Apelido":nu.get("apelido","—"),
                                "CNPJ":"—","Consumo Comp.":round(cc,2),"Saldo":0,"Necessidade":round(cc,2)})

                tn=sum(r["Necessidade"] for r in res)
                for r in res: r["Rateio %"]=round((r["Necessidade"]/gkwh*100),4) if gkwh>0 else 0

                soma=sum(r["Rateio %"] for r in res); sobra=round(100.0-soma,4)
                if sobra!=0:
                    ...
                    if ia is not None: res[ia]["Rateio %"]=round(res[ia]["Rateio %"]+sobra,4)
                    elif tn>0:
                        for r in res: r["Rateio %"]=round(r["Rateio %"]+sobra*(r["Necessidade"]/tn),4)
                ...
                st.download_button("Exportar Excel",df_to_excel_bytes(df_show),
                    f"rateio_{sel_ger}_{sel_u}.xlsx",
    ...
    with tb:
        ...
        gv=clean_val(ug2[nm2.index(su2)].get("geracao_estimada",0))
        ...
        if f_u and st.button("Salvar como Vigente"):
            try:
                ...
                hist[ch].append({"versao":len(hist[ch])+1,
                                 ...
                                 "geracao_kwh":gv,"dados":df_u.to_dict(orient="records")})
                save_rateios(hist); st.success(f"Rateio v{len(hist[ch])} salvo — **{su2}**")
            except Exception as e: st.error(str(e))
    ...
    with td:
        ...
        st.markdown("Pesquisa em todos os rateios vigentes do sistema.")
        ...
        if buscar:
            if not uc_b.strip():
                ...
                unb=normalize_uc(uc_b); hist=load_rateios(); bo=load_backoffice(); found=False
                    ...
                    for row in vig["dados"]:
                        for k,v in row.items():
                            if normalize_uc(str(v))==unb:
                                ...
                                pt_k=next((k2 for k2 in row if "%" in k2 or "rateio" in k2.lower()
...
""")
                                if bo_r:
                                    ...
                                    b3.metric("Tipo",bo_r.get("tipo_instalacao","—"))
                                else:
...
def main():
    ...
    if "user" not in st.session_state:
        ...
        with mid:
            ...
            st.markdown('<div class="login-title">Hub Operacional</div>',unsafe_allow_html=True)
            st.markdown('<div class="login-sub">Plataforma de gestão de geração distribuída</div>',
                        unsafe_allow_html=True)
            with st.form("login"):
